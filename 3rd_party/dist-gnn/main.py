...
def train(cfg: DictConfig, client: Optional[OnlineClient] = None) -> None:
    ...
    if cfg.online:
        ...
        if RANK == 0:
            ...
            if len(trainer.online_timers["streamSlack"]) > 0:
                min_val, max_val, avg_val = utils.min_max_avg(
                    trainer.online_timers["streamSlack"]
                )
                last_step = trainer.online_timers["streamStep"][-1]
                log.info(
                    f"\tStream slack [sec]: min={min_val:.4g}, max={max_val:.4g}, mean={avg_val:.4g}"
                )
                log.info(f"\tLast stream step: {last_step}")

    # Print FOM
    gnn_fom = (
        (global_stats["n_nodes"] / 1.0e6) * (trainer.iteration - 1) / sum(local_time)
    )
